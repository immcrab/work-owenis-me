-- Direct, no-AI submission path: anyone can add a link and it's inserted
-- immediately under category "Unofficial". Replaces the earlier
-- screenshot+vision-model check (that edge function is retired) with a
-- single SECURITY DEFINER RPC, same trust model as record_visit/cast_vote —
-- the anon key can only call this function, not write to `games` directly.
